...
function display_top_n(n, input, search_result) {
    ...
    const top_n = search_result_top_n(n, input, search_result);

    document.getElementById("top10").innerHTML = top_n.join("<br/>")
}

function search_result_top_n(n, input, search_result) {
    let top_n = [];
    for (let i = 0; i < Math.min(n, search_result.length); i++) {
        top_n.push(Math.round(search_result[i][1]*100)/100 + " | " + highlightTXT(input, list_of_idioms[search_result[i][0]])) 
    }
    return top_n;
}
